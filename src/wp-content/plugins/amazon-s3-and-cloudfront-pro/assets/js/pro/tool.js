(function( $, _ ) {
	...
	as3cfpro.tool = {
		...
		/**
		 * Dismiss all tool errors for an item.
		 *
		 * @param event Event
		 */
		dismissAllItemErrors: function( event ) {
			...
			var sourceType = $media.data( 'sourceType' );
			var sourceId = $media.data( 'sourceId' );
			...
			as3cfpro.tool.ajaxDismissItemErrors( {
				...
				$( '[data-action="dismiss-item-errors"]', '.as3cf-' + as3cfpro.tool.ID + '-notice-list .media-error-' + sourceType + '-' + sourceId ).fadeOut( 'slow' );

				var $mediaError = $( '.media-error-' + sourceType + '-' + sourceId, '.as3cf-' + as3cfpro.tool.ID + '-notice-list' );
